...
1. **Architectural integration on a SAG circuit.** No published study integrates a regulatory-layer DO, an upper-layer augmented EKF/UKF (or MHE), and an economic stage cost into a single closed-loop EMPC on a SAG mill. Every DO + MPC mill paper uses a tracking cost; every EMPC mill paper uses an offset-free augmented KF or MHE without a fast disturbance-observer feed-forward channel; the composite is unbuilt.

2. **Realistic SAG-mill operating conditions in EMPC evaluation.** Industrial SAG mills settle at _operational steady state_ often, but the underlying conditions never do: ore hardness drifts on hour-to-shift timescales and liner wear changes the mill geometry over weeks to months, so the plant moves between mildly suboptimal operating points rather than running at a single fixed economic optimum. Published EMPC studies on grinding circuits, such as Thivierge et al. (2023), report performance under tightly controlled conditions: step disturbances applied at fixed operating points, with metrics sampled after the simulator re-settles. How a SAG-mill EMPC behaves under disturbance sequences that resemble actual plant operation, rather than at fixed steady states, is therefore an open empirical question. This is an empirical gap.

3. **Cost-function scope on a SAG circuit.** The plant-wide framework of Le Roux & Craig (2019) and the flotation-revenue cost of Bouchard / Thivierge (2023) both argue that the mill's economic objective should pull from downstream plant signals rather than be tuned mill-internally; but neither has been instantiated as the cost function of a closed-loop EMPC on a single-stage SAG mill. This thesis will explore a wider family of cost-function options.
